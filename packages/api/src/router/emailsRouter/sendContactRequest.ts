...
import { type MailDataRequired, sendEmails } from "@laitjy/sendgrid";
import { z } from "zod";
...
export const sendContactRequestEmail = publicProcedure
  ...
  .mutation(async ({ input }) => {
    ...
    await sendEmails([contactEmail, responseEmail]);
  });

type Input = RouterInputs["email"]["sendContactRequest"];
