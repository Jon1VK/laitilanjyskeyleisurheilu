...
import type { Event, EventType } from "@laitjy/db";
import { dateUtils, eventUtils } from "@laitjy/utils";
...
const createEventCalendarNavigator = (
  ...
  const calendarDates = createMemo(() =>
    dateUtils.getCalendarDates(year(), month())
  );

  const [eventsResource, { refetch }] = createResource(
    () => ({
      startDate: dateUtils.getMonthStartDate(year(), month()),
      endDate: dateUtils.getMonthEndDate(year(), month()),
    }),
  ...
  const eventsByDate = createMemo(() => {
    ...
    return eventUtils.mapEventsByDate(events, monthDates);
  });
  ...
  const createRecurringEvent = async (formData: FormData) => {
    ...
    const startTime = dateUtils.toUTCTime(
      formData.get("startDateTime") as string
    );
    const endTimeValue = formData.get("endDateTime") as string;
    const endTime = endTimeValue ? dateUtils.toUTCTime(endTimeValue) : null;
    const title = formData.get("title") as string;
    const location = (formData.get("location") as string) || null;
    const description = (formData.get("description") as string) || null;
    const pressStartBefore = Number(formData.get("pressStartBefore") as string);
